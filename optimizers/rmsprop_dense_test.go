...
func TestRMSProp_Dense_1(t *testing.T) {
	...
	optimizers := optimizers.NewRMSProp(0.1, 0.0, 1e-8)
	...
	result_weights_1 := dense_1.GetWeights()
	result_weights_2 := dense_2.GetWeights()
	target_weights_1 := mat.NewDense(2, 3, []float64{0.9, 2, 0.1, -2.9, 2, 2.9})
	target_weights_2 := mat.NewDense(3, 2, []float64{-2.1, 2.9, 0.9, 0.9, 1, -3})

	result_bias_1 := dense_1.GetBiasData()
	result_bias_2 := dense_2.GetBiasData()
	target_bias_1 := []float64{2.1, -5.1}
	target_bias_2 := []float64{-0.1, -0.1, 0}

	if !functools.IsEqualMat(target_weights_1, result_weights_1, 0.001) {
		fmt.Println("== I WEIGHTS ==")
		functools.PrintMat(target_weights_1, 2)
		fmt.Println()
		functools.PrintMat(result_weights_1, 2)
		t.Fail()
	}
	if !functools.IsEqualMat(target_weights_2, result_weights_2, 0.001) {
		fmt.Println("== II WEIGHTS ==")
		functools.PrintMat(target_weights_2, 2)
		fmt.Println()
		functools.PrintMat(result_weights_2, 2)
		t.Fail()
	}
...
func TestRMSProp_Dense_Momentum_1(t *testing.T) {
	...
	targetWeights1 := mat.NewDense(2, 3, []float64{2.684, 2.316, 0.684, -3, 2, 2})
	resultBias1 := dense1.GetBias()
	targetBias1 := mat.NewVecDense(2, []float64{1.684, -2})
	resultWeights2 := dense2.GetWeights()
	targetWeights2 := mat.NewDense(1, 2, []float64{0.684, 2})
	resultBias2 := dense2.GetBias()
	targetBias2 := mat.NewVecDense(1, []float64{-0.316})

	if !functools.IsEqualMat(targetWeights1, resultWeights1, 0.001) {
		fmt.Println("== I WEIGHTS == ")
		functools.PrintMat(targetWeights1, 3)
		functools.PrintMat(resultWeights1, 3)
		t.Fail()
	}
	if !functools.IsEqualMat(targetWeights2, resultWeights2, 0.001) {
		fmt.Println("== II WEIGHTS == ")
	...
	input2 := mat.NewVecDense(3, []float64{-2, 1, 1})
	output2 := dense1.Forward(input2)
	activated2 := act.Forward(output2)
	dense2.Forward(activated2)

	optimizer.BackwardDenseLayers(&denses, mat.NewVecDense(1, []float64{2}))

	resultWeights1_2 := dense1.GetWeights()
	targetWeights1_2 := mat.NewDense(2, 3, []float64{2.684, 2.316, 0.684, -2.684, 1.684, 1.684})
	resultWeights2_2 := dense2.GetWeights()
	targetWeights2_2 := mat.NewDense(1, 2, []float64{0.684, 1.684})
	resultBias1_2 := dense1.GetBias()
	targetBias1_2 := mat.NewVecDense(2, []float64{1.684, -2.3162})
	resultBias2_2 := dense2.GetBias()
	targetBias2_2 := mat.NewVecDense(1, []float64{-0.439})

	if !functools.IsEqualMat(targetWeights1_2, resultWeights1_2, 0.001) {
		fmt.Println("== I WEIGHTS == ")
		functools.PrintMat(targetWeights1, 3)
		functools.PrintMat(resultWeights1, 3)
		t.Fail()
	}
	if !functools.IsEqualMat(targetWeights2_2, resultWeights2_2, 0.001) {
		fmt.Println("== II WEIGHTS == ")
		functools.PrintMat(targetWeights2, 3)
		functools.PrintMat(resultWeights2, 3)
		t.Fail()
	}
	if !functools.IsEqualVec(targetBias1_2, resultBias1_2, 0.001) {
		fmt.Println("== I BIAS ==")
		fmt.Println(targetBias1)
		fmt.Println(resultBias1)
		t.Fail()
	}
	if !functools.IsEqualVec(targetBias2_2, resultBias2_2, 0.001) {
		fmt.Println("== II BIAS ==")
		fmt.Println(targetBias2)
		fmt.Println(resultBias2)
		t.Fail()
